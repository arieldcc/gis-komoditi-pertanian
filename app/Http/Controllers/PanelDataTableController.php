...
class PanelDataTableController extends Controller
{
    ...
    private function adminLaporanList(Request $request): JsonResponse
    {
        ...
        return $this->respond(
            ...
            'laporan_pimpinan.generated_at',
            function ($row): array {
                $documentUrl = route('laporan.preview_pdf', ['laporanId' => $row->id], true);
                $isPdf = $this->isPdfDocument($documentUrl);
                ...
                return [
                    ...
                    'file_url' => $this->documentLinkHtml($documentUrl, $isPdf ? 'Lihat PDF' : 'Buka Dokumen'),
                    'actions' => $this->actionsHtml($this->payload([
                        ...
                        'fields' => [
                            ...
                            ['label' => 'URL PDF', 'value' => $documentUrl],
                        ],
                        'document_url' => $documentUrl,
                        'document_type' => $isPdf ? 'pdf' : 'link',
                        'document_label' => $isPdf ? 'Preview PDF Laporan' : 'Buka Dokumen',
                    ])),
                ];
            }
        );
    ...
    private function resolveDocumentUrl(string $fileUrl): string
    {
        ...
        if (str_starts_with($fileUrl, 'http://') || str_starts_with($fileUrl, 'https://') || str_starts_with($fileUrl, '/')) {
            return $fileUrl;
        }
    ...
    private function isPdfDocument(string $documentUrl): bool
    {
        return str_ends_with(strtolower(parse_url($documentUrl, PHP_URL_PATH) ?: $documentUrl), '.pdf');
    }

    private function documentLinkHtml(string $documentUrl, string $label): string
    {
        if ($documentUrl === '') {
        ...
        return (string) new HtmlString(
            '<a href="'.e($documentUrl).'" target="_blank" rel="noopener noreferrer" class="btn btn-outline-danger btn-sm">'.e($label).'</a>'
        );
    }
